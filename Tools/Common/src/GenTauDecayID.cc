...
{
  std::cout<<"enter tauDecayType lalala2"<<std::endl;
					    ...
					    const bool countKShort, const bool applyPTCuts) const
{
  ...
  for (unsigned int iDaughter = 0; iDaughter < momRef->numberOfDaughters(); 
       ++iDaughter) {
    std::cout<<"numberofDaughter"<<momRef->numberOfDaughters()<<std::endl;
    const reco::GenParticleRef kidRef = momRef->daughterRef(iDaughter);
    const unsigned int absDaughterPDGID = fabs(kidRef->pdgId());
    std::cout<<"absDaughterPDGID"<<"\t"<<absDaughterPDGID<<std::endl;
  ...
  std::cout<<"num of muons in numAndP4VisibleDecayProducts"<<"\t"<<nMuons<<std::endl;
  return p4;
}
